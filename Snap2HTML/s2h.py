...
def generateDirArray(dirToScan):
    ...
    for currentDir, dirs, files in os.walk(dirToScan):
        ...
        currentDirModifiedTime = currentDirModifiedTime.strftime(
            "%d/%m/%Y %H:%M:%S")
        ...
        for file in files:
            ...
            currentDirArray.append(file+'*'+str(fileSize)+'*'+fileModifiedTime)
        # append total file size to currentDirArray
        ...
        for dir in dirs:
            numDirs = numDirs + 1
            dirLinks = dirLinks + \
                str(dirIDsDictionary[currentDir+'/'+dir]) + '*'
        dirLinks = dirLinks[:-1]    # remove last *
        currentDirArray.append(dirLinks)
        # store currentDirArray on the correspondent position of allDirArray
...
if len(sys.argv) < 3:  # check if required arguments are supplied
    ...
    if os.path.exists(pathToIndex):  # check if the specified directory exists
        ...
        generateHTML(dirData, appName, appVer, genDate, genTime, title,
                     appLink, numFiles, numDirs, grandTotalSize, linkFiles)
    else:
